  ...
  try {
    ...
    return NextResponse.json({
      report: {
        ...
        wordCount: visible.content?.split(/\s+/).filter(Boolean).length ?? 0,
      },
      ...
      access: {
        previewApplied: visible.access === 'preview',
        tableOfContents: entitlements.tableOfContents,
        exportFormats: entitlements.exportFormats.map((format) => format === 'markdown' ? 'md' : format),
        canEditReport: entitlements.canEditReport,
        canRegenerate: entitlements.monthlyRegenerations > 0,
      },
    });
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    if (sectionId && sectionContent) {
      ...
      const visible = applyReportVisibility({ sections: sections as ReportSection[], content: newFullContent }, entitlements);
